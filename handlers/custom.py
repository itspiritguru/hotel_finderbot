...
from api_requests.request_api import request_custom
...
@exception_handler
def custom_logic(call: CallbackQuery, result_hotels: List[Dict], result: List) -> Union[List[Dict], bool]:
    """
    ...
    custom_additional_request, для повторного запроса на следующей странице. Чтобы пользователь долго не ожидал
    информацию, к API делается ещё два дополнительных запроса по отелям, если они не набирается необходимое
    ...
    for hotel in result_hotels:
        distance = hotel['destinationInfo']['distanceFromDestination']['value'] * 1.8
...
@exception_handler
def custom_additional_request(call: CallbackQuery, result) -> None:
    """
    Функция - обращается к файлу request_api, функции request_custom.
    Полученный ответ от API, отправляет в custom_logic
    ...
    custom_logic(call, result_hotels, result)