...
def history_menu(message: Union[Message, CallbackQuery]) -> None:
    """
    Функция выводит пользователю меню раздела History в виде inline-кнопок.
...
def callback_history_menu(call: CallbackQuery) -> None:
    """
    Функция - обработчик  inline-кнопок. Реагирует только на эл менты списка HISTORY_LIST.
    В случае выбора пользователем - Просмотреть, выводит клавиатуру подменю выбора режима просмотра истории.
